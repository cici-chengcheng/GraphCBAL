...
class GraphLoader(object):
    ...
    def _loadGraph(self, header = True):
        ...
        with open(file_name) as f:
            ...
            if header:
                ...
                L = L[1:]
            edge_list = [[int(x) for x in e.strip().split()] for e in L]  # (边数, 2)
            nodeset = set([x for e in edge_list for x in e])  # 节点集合
            # if header:
    ...
    def _toTensor(self,device=None):
        if device is None:
            if self.sparse:
                self.adj = sparse_mx_to_torch_sparse_tensor(self.adj).cuda()  # 从系数矩阵变为稀疏张量
    ...
    def _getNormDeg(self):
        ...
        normdeg =self.deg/ self.deg.max()   # (节点数,)
        return normdeg  # 归一化度
